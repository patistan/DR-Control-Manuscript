function [RespMatrix,RespMatrixTraining,RespMatrixTesting,RespMatrixAllTrials,ProbDist,BinThresh,removedBlocks_new,selected_cells] = get_prob_dist_ver8_for10_pooled_hard_v2(ExpID,selected_cells,bins,folds,test_set,p)
...
%   selected_cells = cells that were used in this decoding
    ...
    response_matrix_all = dataOut.responseMatrix_1(:,:,p); % FOR SECOND OF PAIR
    
    %get bad blocks of all neurons
    %removedBlocks_all = squeeze(dataOut.isRemovedBlock(:,:,:)); % FOR ALL STIM
    removedBlocks_all = squeeze(dataOut.isRemovedBlock(:,:,p)); % FOR SECOND OF PAIR
    dataOut.totalNumStimuli=2;% FOR FIRST OF PAIR
