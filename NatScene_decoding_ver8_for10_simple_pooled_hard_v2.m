function [Pair_data,selected_cells] = NatScene_decoding_ver8_for10_simple_pooled_hard_v2(ExpID,type,selected_cells,bins,folds,pairs)
...
for fold = 1:folds
    ...
    [RespMatrix,RespMatrixTraining,RespMatrixTesting,RespMatrixAllTrials,ProbDist,BinThresh,removedBlocks_new,selected_cells]=get_prob_dist_ver8_for10_pooled_hard_v2(ExpID,selected_cells,bins,folds,fold,p);
...
Pair_data{pair,1} = AllFold_AllBins;
Pair_data{pair,2} = bins_accuracy;
...
save(sprintf('NBdecoding_%istim_n%i_%s.mat',size(RespMatrix,3),length(selected_cells),type),'Pair_data','selected_cells');
fprintf('pair %d finished\n',pair);
end
